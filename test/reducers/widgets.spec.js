...
describe('reducer: widgets', () => {
  ...
  describe('ADD_WIDGET', () => {
    it('adds a widget to the widget collection', () => {
      const initialState = {}
      const action = { type: ADD_WIDGET, payload: {id: 'widget_1', widgetType: 'Text'} }
  ...
  describe('UPDATE_WIDGET_POSITION', () => {
    it('moves widgets by an absolute amount when {relative: false}', () => {
      ...
      const action = { type: UPDATE_WIDGET_POSITION, payload: {ids: ['w1'], relative: false, changes: newPosition} }
      const finalState = widgets(initialState, action)
    ...
    it('moves widgets by a relative amount when {relative: true}', () => {
      ...
      const action = { type: UPDATE_WIDGET_POSITION, payload: {ids: ['w1'], relative: true, changes: newPosition} }
      const finalState = widgets(initialState, action)
