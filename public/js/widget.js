...
window.Widget = (function() {
  ...
  function Widget(config, data, subject) {
    ...
    zIndex = config.z_index != null ? config.z_index : this.content.defaultZIndex();
    this.origin = {
      x: config.x != null ? config.x : (960 / 2) - (width / 2),
      y: config.y != null ? config.y : $('#viewport').scrollTop() + 100,
      zIndex: zIndex,
      width: width,
      height: height
    };
  ...
  Widget.prototype.moved = function() {
    ...
    this.cachePosition();
    return Designer.history.push(this, 'moveTo', oldPosition, this.position, Designer.template.currentPageNumber);
  };
  ...
  Widget.prototype.nudge = function(x, y) {
    ...
    this.cachePosition();
    Designer.history.push(this, 'moveTo', oldPosition, this.position, Designer.template.currentPageNumber);
